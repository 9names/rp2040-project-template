...
// TODO: convert all the existing stuff to defmt format once verified
...
fn ep0_in_handler(usb: &mut UsbDevice, _buff: &[VolatileCell<u8>], _len: u32) {
    ...
    if usb.should_set_address {
        ...
        debug!(" - Device address set to {:?}", usb.dev_addr);
...
impl UsbDevice {
    ...
    pub fn poll(&mut self) {
        ...
        if raw_interrupts.bits() != 0 {
            ...
            write!(&mut buf, "{:02x}", raw_interrupts.bits()).unwrap();
            debug!("INTS: {:?}", buf.as_str());
        }
    ...
    fn handle_buff_done(&mut self, endpoint_index: u8, input: bool) {
        ...
        write!(&mut buf, "{:02x}", ep_addr).unwrap();
        debug!("- Buffer for EP {:?}", buf.as_str());
    ...
    pub fn start_transfer(&mut self, endpoint_addr: u8, len: u32, data: Option<&[u8]>) {
        ...
        write!(&mut buf, "{:02x}", endpoint_addr).unwrap();
        debug!("Transfer: EP={:?}, len={:?}", buf.as_str(), len);
...
fn usb_setup_endpoint(ep: &EndpointConfig) {
    ...
    write!(&mut buf, "{:02x}", ep.descriptor.bEndpointAddress).unwrap();
    info!("EP {:?}:", buf.as_str());
    buf.clear();

    write!(&mut buf, "{:?}", ep.data_buffer.as_ptr()).unwrap();
    info!("EP DATA:     {:?}", buf.as_str());
    buf.clear();

    write!(&mut buf, "{:?}", ep.buffer_control).unwrap();
    info!("EP BUFF_CTL: {:?}", buf.as_str());
    buf.clear();

    if let Some(endpoint_control) = ep.endpoint_control {
        let dpram_offset = usb_buffer_offset(ep.data_buffer.as_ptr() as _);

        write!(&mut buf, "{:x}", dpram_offset).unwrap();
        info!("Offset {:?}", buf.as_str());
        buf.clear();

        write!(&mut buf, "{:?}", endpoint_control).unwrap();
        info!("EP CTRL: {:?}", buf.as_str());
        buf.clear();
